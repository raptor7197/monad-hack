    ...
    <section aria-label="Contract Deployment" className="w-full border-b border-border bg-surface-2 py-10 md:py-14">
      <div className="mx-auto max-w-[1440px] px-6 md:px-16 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10">
        <h2
          ...
          style={{ fontSize: "clamp(20px, 3vw, 40px)", letterSpacing: "-0.04em" }}
        >
          GOVERNANCE CONTRACT
        </h2>

        <div className="w-full max-w-2xl flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full flex-1 rounded-2xl border-2 border-border bg-surface px-4 py-3 flex items-center justify-between overflow-hidden">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              onClick={copyAddress}
              className="flex-1 sm:flex-none font-display font-extrabold text-xs uppercase px-5 py-3 rounded-2xl bg-brand text-brand-contrast hover:bg-brand-hover transition-colors whitespace-nowrap"
            >
            ...
            {governance.address && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="font-display font-extrabold text-xs uppercase px-4 py-3 rounded-2xl border border-border bg-surface hover:border-brand text-muted hover:text-ink transition-colors"
