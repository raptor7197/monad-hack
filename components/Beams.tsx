...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: ExtendMaterialCfg) {
  const physical = THREE.ShaderLib.physical;
  const { vertexShader: baseVert, fragmentShader: baseFrag, uniforms: baseUniforms } = physical;
  const baseDefines = (physical as unknown as { defines?: Record<string, unknown> }).defines ?? {};

  const uniforms: Record<string, { value: unknown }> = THREE.UniformsUtils.clone(baseUniforms);
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    uniforms[key] = u !== null && typeof u === "object" && "value" in (u as Record<string, unknown>)
      ? (u as { value: unknown })
      : { value: u };
  });
...
const DirLight = ({ position, color }: { position: [number, number, number]; color: string }) => {
  ...
  useEffect(() => {
    ...
    const cam = dir.current.shadow?.camera as THREE.OrthographicCamera | undefined;
    if (!cam) return;
    cam.top = 24;
    cam.bottom = -24;
    cam.left = -24;
...
export function Beams({
  ...
  if (!mounted) {
    return <div className="w-full h-full bg-bg" />;
  }
